...
## 2026-08-15 grill-me 拆解 + 中文版 wk-grill（文章已上線）

網址
https://weken.news/articles/claude-code-grill-me-chinese-version-wk-grill

**切角**：不是介紹文，是拆解加自製。中文圈已經有 Threads 分享、翻譯站、zh 文件站寫過 grill-me，
但全部停在「這個很好用，推薦你裝」。沒有人講機制，也沒有人發現它改版了。

**三個第一手**
1. grill-me 全文只有 5 行，正文一句 `Run a grilling session`，邏輯在另一支 grilling 裡
2. 它 2026-07-16 的 commit `round-by-round frontier interview` 把「一次問一題」改成「一圈一批」，
   中文圈的說法全是舊的。**包含站上自己 7/23 那篇 claude-code-skill-writing-matt-pocock-6-layer 也寫錯**，
   這篇裡直接更正自己，沒有裝作沒發生
3. 當天照它的架構做出中文版 wk-grill（120 行、7 步、7 個完成判準），
   多了動機過濾、手機閱讀格式、自動落地決策表、開場分等級，另外補了原版沒有的三個認輸條件

**G7 誠實框架**：明講當天做的是拆解與自製，不是長期使用心得，
沒有寫任何「用它省了幾小時」的數字。

**交付檢查（都跑過）**：em dash 0、星號 0、`--` 分段、`---` 只有 frontmatter 2 處、
無 CTA 結尾、description 126 字、FAQ 5 題、HowTo 3 步、GEO 引用區塊 2 個、
數字交叉一致（218,065 星 / 78 萬安裝 / 120 行 / 7 步 / 35 支 / 7 月 16 日）、
`npm run build` 通過、線上回 200 並抓頁面確認標題與內文都對。

**這篇的可複製點**：更正自己站上舊文章的錯誤，本身就是一個差異點。
別人不會去更正別人的文章，只有你會更正自己的。而且 repo 更新得比文章快，
「回去看 commit 紀錄」是一個中文圈幾乎沒人做的動作。

**待辦**：aeo 的 voice 樣本庫仍然是空的，這篇是靠 references/style.md 寫的。
他哪天說「voice 加」就把滿意的文章存成樣本。

--

## 第 86 篇：LINE 社群（OpenChat）可以放機器人嗎（2026-08-19）

`src/content/articles/line-openchat-bot-messaging-api-not-supported.md`

**起點**：週末哥在 TG 問「我看到有人做 LINE bot 放 LINE 社群，我記得以前只能放群組」，
追問後補一句「他說是用私人帳號做的」。

**切角**：不是教學文，是把「官方沒說不行」跟「官方說可以」中間那塊空白補起來。
搜「LINE 社群 機器人」出來的九成在教怎麼建社群，一成說不能用機器人，
沒有人講官方其實已經給了兩條路。

**五個查到的第一手**
1. Messaging API 文件只列一對一、群組、多人聊天室三種，全文沒出現 OpenChat
2. 日本開發者 yuppejp 2019-08-19 實測邀 echo-bot 進社群，連 join 事件都不觸發；
   line-bot-sdk-python issue #918 問同一件事，無官方回覆就關閉
3. 社群內部代號是 Square 不是 Group，CHRLINE 逆向專案裡有整組 SquareService，
   甚至有 SquareBotService，代表機制存在但沒對外開放
4. 社群內建三個自動程式（翻譯／垃圾訊息過濾器／自動回應），
   關鍵字要用斜線觸發、一組綁 30 個，這點幾乎沒有中文文章寫過
5. 2026-01-19 官方開放社群綁定開店幫手，首購自動加官方帳號好友，
   綁定期間社群管理員權限無法轉移

**G7 誠實框架**：這是查證型不是實測型。全篇沒有第一人稱使用數據，
所有實測都標明是別人做的並附時間，我做的第一手是「把官方文件逐條查過並交叉比對」。

**交付檢查（都跑過）**：em dash 0、星號 0、`--` 分段 6 處、`---` 只有 frontmatter、
無 CTA 結尾、FAQ 5 題、HowTo 3 步、GEO 引用區塊 3 個、全形逗號 122 半形 4、
數字交叉一致（三種聊天室／30 個關鍵字／2026-01-19／條款 13 與 14.2／5 分鐘 4 群組 2 則）、
`npm run build` 通過、dev server 回 200 且 FAQPage 與 HowTo 與 3 個 Quotation schema 都有渲染、
375 與 1024 截圖打開看過，無橫向捲動無內部溢出。

**這篇的可複製點**：「官方沒有明文說不支援」的題目最好寫。
因為沒有一份文件可以搜到答案，所有人只能靠別人的實測拼湊，
而拼湊出來的東西又停在好幾年前。把官方文件的沉默本身寫成答案，就是差異點。

**待辦**：aeo 的 voice 樣本庫仍然是空的，這篇一樣是靠 references/style.md 寫的。
另外注意 08-15 那篇 grill-me 用了全半形混排（半形逗號 109 個），
跟前後五篇的全形慣例不一致，之後要不要統一回去可以問他一次。


--

## 2026-08-29 兩篇新文章：迴圈工程審自己的產線 ＋ 台灣人怎麼用 Claude

兩篇都是同一晚寫的，起點是他一句「我的AI剪片skill 血汗小組 算是一種loop engineering嗎」。

### 第一篇 loop-engineering-audit-real-video-pipeline

拿 2026 年主流的迴圈工程 7 項檢查清單，逐項審自己那條 9 棒剪片產線，5 項對上、2 項缺口。

第一手素材全部從 `~/.claude/skills/wk-sweat-crew/SKILL.md` 挖出來：
9 棒、閘在第 5 棒、字幕閱讀速率 6.5 警告 8.5 退回、推鏡進出都是 34.57 秒、
閘前移省下的每次 40 秒乘兩輪、2026-08-05 那個整輪校對零進成品的事故。

**開場用事故不用定義**：校對把 cloud 改成 Claude，剪輯跑完字幕還是舊的。
那個事故剛好是清單上最貴的失敗模式（假裝做完），拿它當鉤子比解釋名詞有力。

**結尾放了一條四份資料都沒講的**：宣告層不等於執行層，規則是宣告，檢查才是執行。
這是這篇跟網路上其他概念文最不一樣的地方。

誠實標註：437 美金那個事故查不到原始出處，文內明寫「當警示看就好」；IBM 來源標二手。

### 第二篇 taiwan-claude-usage-anthropic-economic-index-raw-data

他丟了 Anthropic 經濟指數的台灣截圖說「幫我找到更多台灣資料」。
我下載 210MB 原始資料集自己算，**抓到中文圈沒人講的一個點：
那頁的 31/121 是人均指數排名，台灣的絕對用量其實排第 23。**

主軸三塊：兩個排名的差別、台灣對全球的四個偏離、最突出的六個題目倍數。
最後接上微軟 31.8% 全球第 20 跟資策會 46%，解釋三份數字為什麼不衝突。

**寫作上唯一的風險是兩個排名混用**，所以交件前逐處 grep 檢查：
所有「第 31」都接人均指數、所有「第 23」都接用量占比，零混用。

英文欄位名（Proactive activity scanning 這類）刻意不翻，因為翻了別人回頭查不到原始欄位。

### 兩篇共同的驗證

破折號 0、星號 0、`---` 只有 frontmatter 兩行、各 3 個 GEO 引用區塊、
`npm run build` 通過、push 後輪詢到 200、
線上頁面驗到 FAQPage 與 HowTo schema、375px 截圖打開看過。

### 待辦

- aeo 的 voice 樣本庫**還是空的**，這兩篇一樣靠 `references/style.md` 寫。
  已經在 TG 提醒他「voice 加」可以補，他沒回應。這是連續第三次提了。
- 兩篇目前沒有互相連結。第一篇講「來回打磨才是把 AI 當共同作業者」，
  第二篇算出「台灣人來回打磨低於全球 3.7 個百分點」，這兩句是同一件事，
  串起來會讓兩篇都更強。等他點頭再串。
